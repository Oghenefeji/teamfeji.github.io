...
import { ArrowUpRight, ChevronLeft, ChevronRight, Heart, ImagePlus, LockKeyhole, MessageCircle, Sparkles, UserRound, Settings2, Trash2 } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((_event, nextSession) => { setSession(nextSession); if (!nextSession) { setPaid(false); setAccountProfile(null); void loadProfiles(false); } });
    loadProfiles(false);
  ...
  function beginPayment(profile: Profile) {
    ...
    if (paid && profile.whatsapp_number) { window.open(whatsappLink(profile.whatsapp_number), "_blank", "noopener,noreferrer"); return; }
    ...
    (window as any).FlutterwaveCheckout({ public_key: publicKey, tx_ref: `RD-${Date.now()}`, amount: 1500, currency: "NGN", payment_options: "card, ussd, banktransfer", customer: { email: session.user.email, name: session.user.user_metadata?.full_name || "Request Date member" }, customizations: { title: "Request Date Premium", description: "Unlimited WhatsApp access for life" }, callback: async (data: any) => {
      if (data.status !== "successful" || !supabase) { setPaymentBusy(false); return; }
      try {
        ...
        setPaid(true); await loadProfiles(true); toast.success("Access unlocked. You can now connect with anyone.");
      } catch (error: any) { toast.error(error?.message || "Payment verification failed."); }
      finally { setPaymentBusy(false); }
    }, onclose: () => setPaymentBusy(false) });
  }
  ...
  return <div className="min-h-screen bg-white text-[#0f172a]">
    ...
    {paymentBusy && <div className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 rounded-full bg-[#0f172a] px-5 py-3 text-sm font-semibold text-white shadow-2xl">Opening secure checkout…</div>}
  </div>;
}
